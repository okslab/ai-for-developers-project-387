...
@router.post("/api/owner/event-types", status_code=201, response_model=EventType)
def create_event_type(body: EventTypeCreate, store: Store = Depends(get_store)):
    return store.create_event_type(body)


@router.get("/api/owner/event-types", response_model=list[EventType])
def list_event_types(store: Store = Depends(get_store)):
    return store.list_event_types()


@router.get("/api/owner/bookings", response_model=list[BookingWithEventType])
